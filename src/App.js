...
export default function App() {
  ...
  function deletePost(postId) {
    const url = `${Constants.API_URL_GET_DELETE_POST_BY_ID}?id=${postId}`;
  ...
  function renderPostsTable() {
    ...
    return (
      <div className="table-responsive mt-5">
        ...
        <button className="btn btn-dark btn-lg w-100" onClick={() => setPosts([])}>Empty post array</button>
      </div>
    )
  }
  ...
  function onPostUpdated(updatePost) {
    ...
    alert(`Post successfully updated. After clicking OK, look for the post with the title "${updatePost.title}" in the table below to see the updates`);
  }
